...
function PickUpOrderPage() {
  ...
  const handleCheckoutSubmit = async (details: any) => {
    ...
    try {
      ...
      try {
        const tripayData = await tripayService.createTransaction(order.id);
        
        toast.success("Pesanan Pickup terkirim!");
        clearCart();
        ...
        localStorage.setItem("lnr_active_order", order.id);
        // Fallback to order page even if tripay fails (maybe they can retry there)
        navigate({ to: "/order/$orderId", params: { orderId: order.id } });
      } finally {
      ...
      <header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex max-w-2xl items-center justify-between px-4 py-4">
          ...
          <div className="text-center">
            ...
            <div className="font-display text-lg font-bold">Pick Up</div>
          </div>
